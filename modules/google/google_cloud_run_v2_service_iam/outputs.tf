...
locals {
  binding = one(google_cloud_run_v2_service_iam_binding.binding)
  iam_output = [
    local.binding,
    local.member,
    local.policy,
  ...
  member           = google_cloud_run_v2_service_iam_member.member
  policy           = one(google_cloud_run_v2_service_iam_policy.policy)
}
output "iam" {
